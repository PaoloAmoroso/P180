...
var RAD2DEG      = 57.2957795;
var DEG2RAD      = 0.016774532925;
var SLAR_Path    = "/instrumentation/radar/pixels/";

var DISP_LENGTH   = 0.0856;     # 3D radar line length in meters
var DISP_DEF      = 100;        # fixed number of plots
var DEV_CALIB     = 1.5;        # vertical calibration
var UPDATE_PERIOD = 0.5;        # radar update every 0.5s

var FT2M          = 0.3048;
var NM2M          = 1852;
var D2R           = math.pi / 180;
var MAX_RANGE_M   = 5 * NM2M;   # radar range 5 NM per 100 plots

var interval_m    = MAX_RANGE_M / DISP_DEF;
var radar_scale   = DISP_LENGTH / MAX_RANGE_M;

# Function to get Elevation at latitude and longitude
var SLAR_get_elevation = func (lat, lon) {
...
var update_SLAR = func {
  ...
  for (var col = 0; col <= 98; col += 2) {
    ...
    var elev = SLAR_get_elevation(pointlat, pointlon);

    setprop(SLAR_Path ~ "/col[" ~ col ~ "]/elevation", elev);
  }

  # Interpolate the rest of the points in each column

  for (var col = 1; col <= 97; col += 2) {
    var elevprev = getprop(SLAR_Path ~ "/col[" ~ (col - 1) ~ "]/elevation");
    var elevnext = getprop(SLAR_Path~ "/col[" ~ (col + 1) ~ "]/elevation");
...
var SLAR_loop = func {
  update_SLAR();
  settimer(SLAR_loop, UPDATE_PERIOD);
}
...
#================================================================
#                   FORWARD TERRAIN PROFILE Mode
#================================================================
print("*** LOADING FTP ***");

var RAD2DEG      = 57.2957795;
var DEG2RAD      = 0.016774532925;
var FTP_Path     = "/instrumentation/radar/profile/";
...
# Function to get Elevation at latitude and longitude
var FTP_get_elevation = func (lat, lon) {
...
var update_FTP = func {
  ...
  # First get all the points (16x16)
  for (var col = 0; col <= 98; col += 2) {
    # (49 - col)= da destra a sinistra
    # (col - 49)= da sinistra a destra
    var beam_offset_nm = (col - 49) * (range/30);
    var forward_offset_nm = range / 30;

    var base = geo.Coord.new().set_latlon(poslat, poslon);

    var lateral = base.apply_course_distance(heading - 0, beam_offset_nm * NM2M);
    var point = lateral.apply_course_distance(heading, forward_offset_nm * NM2M);

    var pointlat = point.lat();
    var pointlon = point.lon();

    var elev = FTP_get_elevation(pointlat, pointlon);

    setprop(FTP_Path ~ "/col[" ~ col ~ "]/elevation_m", elev);
  }

  # Interpolate the rest of the points in each column

  for (var col = 1; col <= 97; col += 2) {
    var elevprev = getprop(FTP_Path ~ "/col[" ~ (col - 1) ~ "]/elevation_m");
    var elevnext = getprop(FTP_Path~ "/col[" ~ (col + 1) ~ "]/elevation_m");
    var elev = (elevprev + elevnext) / 2;
...
var FTP_loop = func {
  update_FTP();
  settimer(FTP_loop, UPDATE_PERIOD);
}
